...
Elm.MyCss.make = function (_elm) {
   ...
   var css = function (_p0) {
      return $Css.stylesheet(A2($Css$Namespace.namespace,"craft-conf",_p0));
   }(_U.list([$Css$Elements.img(_U.list([$Css.width($Css.pct(100))]))
             ,A2(F2(function (x,y) {    return A2($Css._op["."],x,y);}),Pointer,_U.list([$Css.margin($Css.auto)]))]));
   var MyImage = {ctor: "MyImage"};
   var _p1 = $Html$CssHelpers.withNamespace("craft-conf");
   var $class = _p1.$class;
   return _elm.MyCss.values = {_op: _op,$class: $class,MyImage: MyImage,Pointer: Pointer,css: css};
};
Elm.Hello = Elm.Hello || {};
Elm.Hello.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var pointerView = function (_p0) {
      var _p1 = _p0;
      return A2($Html.div,
      _U.list([$MyCss.$class(_U.list([$MyCss.Pointer]))]),
      _U.list([$Html.text(A2($Basics._op["++"],
      "x = ",
      A2($Basics._op["++"],$Basics.toString(_p1._0),A2($Basics._op["++"],", y = ",$Basics.toString(_p1._1)))))]));
   };
   var imageView = function (model) {    return A2($Html.img,_U.list([$Html$Attributes.src("images/left.png")]),_U.list([]));};
   var view = function (model) {    return A2($Html.div,_U.list([]),_U.list([pointerView(model),imageView(model)]));};
